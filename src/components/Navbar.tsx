...
export default function Navbar({ className }: { className?: string }) {
  ...
  return (
    ...
    //       </div>
    //       <div
    //         className={`nav-links duration-500 md:static absolute bg-gray-100 md:min-h-fit min-h-[60vh] left-0 ${isOpen ? 'top-0' : 'top-[-100%]'} md:w-auto w-full flex items-center px-5 md:relative`}
    //       >
    ...
    //         </ul>
    //       </div>
    //       <button onClick={toggleDropdown} className="flex items-center gap-6">

    //       {isOpen ? (
    //        <svg

    //       className="h-10 w-10 text-3xl cursor-pointer md:hidden"
    //       viewBox="0 0 24 24"
    //     > <path
    //           fill="none"
